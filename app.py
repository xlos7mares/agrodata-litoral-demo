...
st.markdown("""
    ...
    .perfil-foto {
        ...
        box-shadow: 0px 0px 25px rgba(57, 255, 20, 0.5);
    }

    /* Métricas: Texto NEGRO sobre fondo BLANCO */
    ...
    h1, h2, h3 { color: white !important; text-shadow: 2px 2px 8px black; }
    .stTabs [data-baseweb="tab-list"] { background-color: rgba(0,0,0,0.85); border-radius: 10px; padding: 8px; }
    </style>
    """, unsafe_allow_html=True)

# 2. ENCABEZADO: LOGO XL (IZQ) | PERFIL PREMIUM ALINEADO (DER)
with st.container():
        ...
        with c_text:
            st.markdown(f"""
                <div class="perfil-info">
                    ...
                    <p style="font-size:12.5px; line-height:1.4; opacity: 0.95;">
...
with tabs[0]:
    ...
    with col1:
        st.metric(
            ...
            help="Satelital Sentinel-2 (ESA). Mide el vigor fotosintético y biomasa activa."
        )
    with col2:
        st.metric(
            label="Humedad Perfil (1m)", value="14.2%", delta="-3.8% CRÍTICO", delta_color="inverse",
            help="NASA SMAP. Escaneo de microondas para reservas hídricas profundas."
        )
    with col3:
        st.metric(
            label="Resistencia Suelo", value="2.5 MPa", delta="Suelo Firme",
            help="NASA Landsat. Capacidad de carga mecánica para infraestructura pesada."
        )
...
with tabs[1]:
    ...
    folium.Polygon(
        ...
        color="#39FF14", weight=6, fill=True, fill_color="#39FF14", fill_opacity=0.3
    ).add_to(m)
    folium_static(m, width=1100)

with tabs[2]:
    st.markdown("#### 🔬 Ciencia Satelital y Validación Institucional")
    
    st.info("Sincronización total con: **MGAP, SNIG, DIEA, INIA e OPYPA (Uruguay)**.")

with tabs[3]:
    st.header("📄 Informe Certificado Completo")
    st.error("🔒 BLOQUEADO: Requiere validación de pago u$s 150 a Leonardo Olivera.")

# 4. PIE DE PÁGINA
st.markdown("""
    <div style="text-align: center; color: white; padding: 40px; background-color: rgba(0,0,0,0.75); border-radius: 15px 15px 0 0;">
        <p>© 2026 AgroData Litoral - Todos los derechos reservados</p>
        <p><b>Director de Proyecto: Leonardo Olivera</b> | Litoral Operaciones Inmobiliarias</p>
    </div>
""", unsafe_allow_html=True)
