...
st.markdown("""
    ...
    /* ELIMINAR CONTENEDORES VACÍOS (SOLUCIÓN AL RECUADRO BLANCO) */
    ...
    .fuente-item { background-color: #f0f4f0; padding: 15px; border-radius: 10px; margin-bottom: 10px; border-left: 5px solid #39FF14; }
    </style>
...
with st.sidebar:
    ...
    if st.button("Ejecutar Escaneo Satelital"):
        st.success("Señal vinculada. Procesando datos...")

# 3. ENCABEZADO CON LOGO XL Y PERFIL PROFESIONAL COMPLETO
with st.container():
...
with tabs[0]:
    ...
    with c2:
        ...
        st.write("📡 **NASA SMAP:** Reserva hídrica profunda.")
    with c3:
...
with tabs[2]:
    st.info("Utilizamos constantes dieléctricas para procesar la señal satelital.")
    st.write("✅ **Sincronización:** MGAP, SNIG, DIEA, INIA e OPYPA (Uruguay).")

# 5. SECCIÓN FINAL: INVERSIÓN ÚTIL (LIMPIA Y DETALLADA)
...
<div class="fuente-item">
    <b>🇪🇺 Fuente ESA Sentinel-2:</b> Imágenes multiespectrales de alta resolución. Misión de la Agencia Espacial Europea que monitorea la vegetación cada 5 días para detectar falta de nitrógeno antes de ser visible.
</div>
<div class="fuente-item">
    <b>🇺🇸 Fuente NASA SMAP:</b> Monitorea la humedad profunda (zona radicular) cada 2-3 días. Utiliza radiometría de banda L para predecir sequías con precisión científica (USDA).
</div>
<div class="fuente-item">
...
<div style="background-color: #1e4d2b; color: white; padding: 25px; border-radius: 15px; text-align:center; margin-top:20px;">
    ...
    <p>Apto para Uruguay y cualquier país del mundo.</p>
